...
- **メール送信** — Resend経由、CC/BCC、In-Reply-Toスレッド返信、添付ファイル対応
- **メール受信** — Cloudflare Email Routing → Worker → D1、raw-first R2永続化（メール紛失ゼロ）
- **受信箱検索** — FTS5全文検索（件名、本文、送信者、認証コード）
- **セマンティック検索** — Workers AI + Cloudflare Vectorizeベクトル検索（キーワード/セマンティック/ハイブリッドモード）
- **ダッシュボードコンソール** — ビジュアルメール管理UI（`mails0.com/console`）
...
- **Webhook通知** — メール受信時にURLへPOST、HMAC-SHA256署名検証付き
- **SSEリアルタイムイベント** — `/api/events`で`message.received`イベントをリアルタイム購読
- **メールボックス分離** — `auth_tokens` D1テーブルによるトークン別バインディング、スコープ制御対応
- **メールボックス一時停止/再開** — メールボックスの処理を一時的に停止
- **抑制リスト** — バウンス/苦情の受信者を自動抑制、ドメインレピュテーション保護
